...
| 统一导入窗口 | 已完成 | PoB Code、WeGame 分享链接共用导入入口；SuperPoE 原生构筑文件使用独立“打开”流程 |
...
| 通货行情 | 可用，待完善 | 交易中心只读批量行情页已接入；国服使用 poecurrency.top，国际服使用 poe2scout，跟随全局默认服务器并自动发现国际服普通赛季；支持缓存、搜索、分类、排序、双计价和来源详情，待第三方接口长期稳定性观察 |
...
| 旧 JSON 导入 | 已移除 | 不再接受粘贴 JSON；原生 `.spoe` 文件通过系统文件选择器或双击打开 |
...
| 计算详情 | 可用，待完善 | 技能详情已展示伤害类型链、点伤、暴击、速度、来源和 breakdown；仍需补齐更多防御来源、警告和跨结果对比 |
...
- 底层通用 XML 节点保留未知字段；类型化访问器不得裁剪或重建未触及 section。
...
- 在当前 `PobBuildObject` 的不可变临时副本中替换候选物品，通过同一 PoB worker 重新计算，不修改用户当前构筑。
...
当前进度（2026-08-07）：第一阶段价格查询已完成。装备面板、装备仓库与游戏剪贴板共用独立置顶窗口、词缀筛选、官方 Search/Fetch 价格列表和分页；尚未完成把候选装备代入构筑后的 DPS/防御提升计算，因此 M6 整体仍进行中。
...
- 将规划器解析/生成做成独立模块，不与 PoB Code 或 SuperPoE 原生构筑格式混用。
...
- 将另一套 PoB Code、WeGame 构筑作为只读比较来源，或直接打开 SuperPoE 原生构筑文件，不覆盖当前编辑构筑。
...
7. 价格查询和装备增量计算稳定后，实施 M5 装备提升购买建议。
8. 用 M0 的 Provider、resolver 和仓库实现 M6 游戏内剪贴板查价浮层，不开发第二套查价逻辑。
