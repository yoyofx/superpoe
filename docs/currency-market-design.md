...
> 状态：C0-C3 已实现；双区服真实数据与桌面/窄窗口已验收，待第三方接口长期稳定性观察
> 更新日期：2026-08-01
...
通货行情作为交易中心的第三个一级 Tab，位于“实时监控”之后：
...
选择 `IsCurrent === true` 的 PoE2 普通赛季（排除名称以 `HC ` 开头或短名以 `hc` 结尾的硬核赛季），并使用其 `ShortName` 请求：
...
不得硬编码当前短名，例如 `runes`。poe2scout 会同时把普通与硬核赛季标记为当前赛季，因此必须明确选择普通赛季；找不到当前普通赛季时请求失败并保留旧缓存，不能猜测选择列表第一项。
...
> 当前实现状态（2026-08-01）：双数据源 Adapter、普通赛季自动选择、主进程请求校验、10 分钟内存/磁盘缓存、窄化 IPC、交易中心 Tab、响应式主表与来源详情均已接入；国服与国际服真实接口以及桌面/窄窗口布局已完成验证。
