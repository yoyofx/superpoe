...
├──────────────────────────────────────────────────────────────┤
│ 基础点伤 → 转换/Gain → Increased → More → 暴击 → 防御 → 平均击中 → DPS │
...
实现约定：收益分析页的技能范围是唯一口径。配置了 Full DPS 时使用 `FullSkillDPS`，未配置时使用所有实际 DPS 大于 0 的技能（排除 DoT）的回退范围。七个区域的最终总 DPS 使用该范围的总结果；由于 PoB2 的转换、暴击和防御明细必须绑定单个技能，区域明细选择该范围内 DPS 最高的代表技能，并在上下文中同时标明“代表技能”和“范围总 DPS”，不把多个技能的不同公式机械相加。
...
技术含义：攻击技能从武器基础伤害开始，法术技能从技能等级基础伤害开始；两者再叠加当前构筑提供的附加点伤和技能基础倍率。
...
图形：主视图将“起始基底 + 附加点伤”按伤害类型合并成一组范围，不把元素类型误认为独立乘区；来源逐项明细放在可展开区域。

```text
物理：武器 80 - 120 + 装备 20 - 30 = 100 - 150
火焰：技能等级 210 - 315
闪电：装备 20 - 30
```
...
技术含义：伤害类型转换、`Gain as Extra` 和随机元素分支。物理、火焰、冰霜、闪电、混沌是最终 DPS 的拆分维度，不单独占一个乘区卡片。
...
转换和 Gain 不显示“占最终 DPS 的百分比”，只显示它们改变了哪些类型或增加了哪些分支，避免错误地把伤害类型当成最终乘数。

### 7.8 最终 DPS 类型明细

物理、火焰、冰霜、闪电、混沌不作为公式流程节点。它们在最终 DPS 区域中显示为经过当前暴击、命中率、敌人防御和输出频率后的实际类型 DPS，并显示占代表技能最终 DPS 的比例。多技能范围下，顶部总 DPS 是整个范围合计，类型明细明确标注为代表技能，避免把不同技能的类型公式机械相加。
...
该节点不是新的伤害乘区，而是暴击、特殊命中和敌人减伤计算后的单次期望伤害结果。

```text
增伤后平均击中 = 非暴击平均伤害 × 非暴击比例
               + 暴击平均伤害 × 暴击比例
```

该值包含基底、附加点伤、转换/Gain、Increased/More、暴击伤害倍率、特殊命中效果，以及当前 `EFFECTIVE` 模式下 PoB2 应用的敌人减伤。它不包含命中率和攻击/施法频率。多技能范围下，节点标注为代表技能的平均击中，不能误认为范围总和。
...
频率明细同时列出 `Speed` 的来源，例如装备、天赋和珠宝提供的 Increased/More 攻速；顶部频率值是 PoB2 计算后的最终结果，不用来源百分比简单相加替代。
...
interface DamageStructureReportData {
  ...
  addedRanges: DamageRange[]
  combinedBaseRanges: DamageRange[]
  baseTotalAverage?: number
