...
> 状态：内部构筑库文件化方案已取消，仅作历史参考；当前不按本文迁移构筑中心的 localStorage
> 更新日期：2026-08-10
...
本文记录 SuperPoE2 曾规划的本地数据目录、文件边界、构筑与仓库格式、版本兼容、迁移、原子写入、自动草稿、备份恢复、安全边界和验收标准。内部构筑库文件化、localStorage 首次迁移和自动草稿不再是当前产品路线；现有 `.spoe` 打开/保存副本能力以及交易仓库、设置和凭据相关设计仍可作为参考。

> 历史范围：本文第 7、13 至 22 节中的构筑文件库、自动草稿、localStorage 首次迁移、BuildRepository 和相关 P0-P5 实施/验收内容均已取消，不得作为当前开发任务或实现门禁。当前构筑中心继续使用现有 localStorage；只有用户明确执行 `.spoe` 打开、保存副本或导入导出时才读写原生构筑文件。

原方案目标是让用户的构筑和交易数据不再依赖 Chromium `localStorage`。该目标对构筑中心已取消；当前构筑中心仍使用现有 localStorage，`.spoe` 仅在用户明确操作时读写。交易仓库、设置和凭据仍按各自现有存储边界维护。
...
5. 构筑中心继续以现有 `localStorage` 为事实来源；`*.spoe` 是用户明确打开、保存副本和导入导出的原生文件，不建立 `builds/` 内部构筑库或目录扫描索引。
