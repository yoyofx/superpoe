...
contextBridge.exposeInMainWorld('pob2Desktop', {
  ...
  exportDiagnosticLog: () => ipcRenderer.invoke('pob2:export-diagnostic-log') as Promise<{ canceled: boolean; filePath?: string }>,
  openDiagnosticLogDirectory: () => ipcRenderer.invoke('pob2:open-diagnostic-log-directory') as Promise<void>,
  openTimer: () => ipcRenderer.invoke('pob2:open-timer') as Promise<void>,
  ...
  detectGameDirectory: (realm: 'cn' | 'global') => ipcRenderer.invoke('pob2:detect-game-directory', realm) as Promise<import('../src/types/gameDirectory.js').GameDirectoryDetectionResult>,
  chooseGameDirectory: (realm: 'cn' | 'global', currentDirectory?: string) => ipcRenderer.invoke('pob2:choose-game-directory', { realm, currentDirectory }) as Promise<import('../src/types/gameDirectory.js').GameDirectoryDetectionResult>,
  setAppContext: (context: { defaultRealm: 'cn' | 'global'; language: 'en' | 'zh-rCN' | 'zh-rTW' | 'ko-KR'; priceCheckEnabled: boolean; priceCheckHotkey: string; gameDirectory?: string }) => ipcRenderer.invoke('pob2:set-app-context', context),
...
contextBridge.exposeInMainWorld('pob2Market', {
  ...
  marketPageCommand: (command: 'focus-filters' | 'search' | 'clear') => ipcRenderer.invoke('market:page-command', command) as Promise<void>,
  focusPageListing: (listingId: string) => ipcRenderer.invoke('market:focus-listing', listingId) as Promise<void>,
  visitPageListing: (listingId: string) => ipcRenderer.invoke('market:visit-page-listing', listingId) as Promise<void>,
