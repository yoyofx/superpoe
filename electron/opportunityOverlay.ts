...
const OVERLAY_HTML = `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src 'unsafe-inline'"><style>
...
</style></head><body><main class="panel"><header class="head"><span><strong id="target-name">购买目标</strong><small id="summary">等待机会</small></span><button data-action="close" title="关闭">×</button></header><section class="body"><aside class="queue" id="queue"></aside><article class="details" id="details"><div class="item-head"><h2 id="item-name"></h2><span class="base" id="item-base"></span><span class="price" id="item-price"></span></div><div class="properties" id="properties"></div><div class="mods" id="mods"></div><div class="meta"><span id="item-age"></span><span id="item-status"></span></div></article></section><footer class="foot"><button data-action="next">下一件</button><button data-action="skip">跳过这件</button><button class="pause" data-action="pause">暂停此目标</button><button data-action="complete">完成目标</button><button data-action="attempt">前往藏身处</button></footer></main></body></html>`
...
export class OpportunityOverlayController {
  ...
  handleAction(action: string): void {
    ...
    if (action === 'close') {
      ...
      this.statusMessage = '正在重新校验挂单'
      this.publish()
      void this.actions.attempt(current.id).then((result) => {
        this.statusMessage = result === 'attempted' ? '已发送藏身处请求，不代表购买成功'
          : result === 'game-offline' ? '游戏未在线，进入游戏后可再次尝试'
            : result === 'unavailable' ? '挂单可能已经失效'
              : '请求失败，请检查登录状态后重试'
